...
fn char_p_to_string<T>(raw: *const T) -> Result<String> {
    let c_string = unsafe { std::ffi::CStr::from_ptr(raw as *const i8).to_owned() };
...
mod onnxruntime {
    ...
    extern_system_fn! {
        ...
        pub(crate) fn custom_logger(
            ...
            let category = unsafe { CStr::from_ptr(category as *const i8) };
            assert_ne!(code_location, std::ptr::null());
            let code_location = unsafe { CStr::from_ptr(code_location as *const i8) }
                .to_str()
                .unwrap_or("unknown");
            assert_ne!(message, std::ptr::null());
            let message = unsafe { CStr::from_ptr(message as *const i8) };

            assert_ne!(logid, std::ptr::null());
            let logid = unsafe { CStr::from_ptr(logid as *const i8) };
